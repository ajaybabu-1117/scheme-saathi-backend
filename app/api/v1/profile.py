...
def get_profile(claims=Depends(get_current_user_claims)):
    user_id = claims["sub"]
    profile = user_repository.get_profile(user_id) or {"user_id": user_id, "preferred_language": "en"}
...
def update_profile(payload: ProfileUpdateRequest, claims=Depends(get_current_user_claims)):
    user_id = claims["sub"]
    saved = user_repository.save_profile(user_id, payload.model_dump(exclude_none=True))
